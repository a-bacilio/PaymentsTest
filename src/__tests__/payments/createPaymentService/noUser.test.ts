...
describe('Payments', () => {
  ...
  describe('POST', () => {
    it('should return status 400 with invalid payment without a user id', async () => {
      const fakeToken = createAuthToken({ id: '1234' });
      const { body } = await supertest(app)
        .post(postRoute)
        .send({ cart: '1jod1p21m1d' })
        .set('Authorization', fakeToken)
        .expect(400);

      expect(body).toEqual({ message: 'a user id is required', type: 'validation' }); // jest
    });
  })
